...
if(scalar(@ignore_metadata) > 0) { 
	if(-d $output_dir) { 
		...
		if($format =~ m/csv/) { 
			...
			print $csvfiles_fh "\"id\",\"bundle_skip\",\"file\",\"filename\",\"md5_checksum\",\"sha1_checksum\",\"sha256_checksum\"\n";
		...
		if($format =~ m/tsv/) { 
			...
			print $tsvfiles_fh "id\tbundle_skip\tfile\tfilename\tmd5_checksum\tsha1_checksum\tsha256_checksum\n";
